...
public class ExpertKubernetesYaml : StepBody
{
    private const string StepName = "ExpertKubernetesYaml";


    private string _prompt = "请你作为一个k8s专家、高级管理员的角色，面对上述用户的目标。\n" +
                             "请你针对用户给出的目标，返回对应的yaml内容。\n" +
                             "要求命令用```yaml ```包裹起来。" +
                             "请务必注意修正yaml的格式，尤其是空格、缩进、引号等，保证必须是可以直接执行的、正确的yaml格式。\n" +
                             "请注意使用<br>换行.请作答";
